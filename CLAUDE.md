...
- `internal/preprocessor/` — AST-based preprocessor package (`run.go` / `compile.go` / `provenstub.go` / `scanner.go` / `analyzer.go` / `rewriter.go` / `userpkg.go`) plus its `e2e_test.go` golden-file harness over `testdata/cases/`, `provenstub_test.go`, `scanner_test.go`, `analyzer_test.go`, `infer_test.go`, and `rewriter_test.go`.
- **Preprocessor behavior: Phases 1–5 done.** Phase 1 injects a companion `.go` file into pkg/proven's compile providing the `_proven_atCompileTime` linker symbol as a no-op. Phase 2 (`ScanPackage`) builds a per-package `PackageSummary{Funcs, Rules}` of parameter preconditions, return postconditions, and inference-rule implications. Phase 3 (`AnalyzeFunc` / `AnalyzePackage`) walks each caller under a mutable `FactSet`, producing `CallDischarge{Params []ParamDischarge{Required, Missing}}` per call; fact sources are preceding `if pred(x)`, early-return/panic guards, conjoined `&&`, `proven.Returns` postconditions, and successful `prove.That` / `prove.Must` — the latter two correctly gate on the err-check's branch. Phase 4 extends discharge with backward-chaining over `infer.From(p).[Given(c).]To(q)` rules, cycle-safe and chained. Phase 5a wires it all into the compile path: `planCompile` returns a `Plan{NewArgs, Cleanup, Diags}`; `planUserPackage` scans+analyzes every user-package compile and emits Go-standard `file:line:col: proven: undischarged predicate <p> on parameter N of <callee>` diagnostics before exiting non-zero. Phase 5b (`rewriter.go`) makes discharged call sites zero-cost: after a clean analysis, every `proven.That` / `proven.Returns` call is blanked in the source bytes the compiler sees. Rewrites are byte-level, length-preserving for all pre-existing lines (cmd/compile error positions still match the user's source column-for-column), and end with a single appended sentinel line that keeps the proven import in use. Fixtures under `testdata/cases/` cover all five fact sources plus both fail cases. Roadmap in [`docs/todo/roadmap.md`](docs/todo/roadmap.md).
