...
- `internal/preprocessor/` — AST-based preprocessor package (`run.go` / `compile.go` / `provenstub.go` / `scanner.go` / `analyzer.go`) plus its `e2e_test.go` golden-file harness over `testdata/cases/`, `provenstub_test.go`, `scanner_test.go`, and `analyzer_test.go`.
- **Preprocessor behavior: Phases 1–4 done.** Phase 1 — stub injection: `compile` invocations for `github.com/GiGurra/proven/pkg/proven` get a companion `.go` file whose signature is derived by AST-parsing the source in the compile's own argv. That file provides the `_proven_atCompileTime` linker symbol as a no-op. Phase 2 — per-package obligation scan: `ScanPackage` returns a `PackageSummary{Funcs, Rules}` with parameter preconditions and return postconditions for every annotated function. Phase 3 — flow-sensitive discharge: `AnalyzeFunc` walks each caller FuncDecl under a mutable `FactSet`, returning one `CallDischarge{Params []ParamDischarge{Required, Missing}}` per call to a summary-known callee. Fact sources: preceding `if pred(x)`, early-return/panic guard `if !pred(x){escape}`, conjoined `&&`, `proven.Returns`-annotated postcondition on assignment, successful `prove.That` / `prove.Must`. Branch-merge keeps the surviving branch's facts when one branch always escapes (return/panic). Phase 4 — inference-rule consumption: scanner harvests `var _ = infer.From(p).[Given(c).]To(q)` into `PackageSummary.Rules`; analyzer's discharge falls back from direct fact lookup to backward-chaining through the rules (cycle-safe via per-query visited set, chained multi-hop, Given contexts enforced on the same variable). Neither scanner nor analyzer is wired into the compile path yet; Phase 5 will do that (rewriting + diagnostics). Roadmap in [`docs/todo/roadmap.md`](docs/todo/roadmap.md).
